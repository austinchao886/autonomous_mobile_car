...
MeamrBase::MeamrBase(std::shared_ptr<SerialInterface> serial)
: Node("meamr_base_node"),
  ...
  serial_interface_(serial)
{
    ...
    cmd_vel_sub_ = this->create_subscription<geometry_msgs::msg::Twist>(
        "cmd_vel_joy", 1, std::bind(&MeamrBase::cmdVelCallback, this, std::placeholders::_1));
    // Timer
...
{   
    ...
    // RCLCPP_INFO(this->get_logger(), "Desired velocities: linear.x: %f, angular.z: %f", des_lin_vel_, des_theta_vel_);
    // 發送速度指令到馬達
...
void MeamrBase::Update()
{
    ...
    if (!rx_data.empty()) {
        ...
        size_t pos2 = data_str.find(',');
        size_t pos3 = data_str.find('\n');
        if (pos1 == 0 && pos2 != std::string::npos && pos3 != std::string::npos && pos2 < pos3) {
            try {
                lin_vel = std::stof(data_str.substr(pos1 + 1, pos2 - pos1 - 1));
                theta_vel = std::stof(data_str.substr(pos2 + 1, pos3 - pos2 - 1));
